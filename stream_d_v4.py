...
def hash_user(label): return "u_" + hashlib.sha256((SALT + label).encode()).hexdigest()[:12] if label else "unknown"
...
for line in CHAT_JSONL.open():
    ...
    if not ts: continue
    c = m.get('content','')
    if isinstance(c, dict): c = c.get('text','') or ''
    m['_ts'] = ts
...
for m in system_msgs:
    c = m['_content_str']; ts = m['_ts']
    mm_inv = JOIN_RE_INV.match(c); mm_qr = JOIN_RE_QR.match(c)
    if mm_inv:
        join_events.append((ts, mm_inv.group(2).strip().rstrip('.'), mm_inv.group(1).strip(), 'invited'))
    elif mm_qr:
        m_qr = re.search(r'QR Code shared by (.+?)\.', c)
        join_events.append((ts, mm_qr.group(1).strip(), m_qr.group(1).strip() if m_qr else None, 'qr'))
...
for m in content_msgs:
    ...
    if uid not in posters:
        posters[uid] = {'name': s.get('name',''), 'messages': 0, 'days': set(),
                        'first': None, 'last': None, 'sender_type': s.get('sender_type','')}
    p = posters[uid]
    p['messages'] += 1
    p['days'].add(m['_ts'].date().isoformat())
    if p['first'] is None or m['_ts'] < p['first']: p['first'] = m['_ts']
    if p['last'] is None or m['_ts'] > p['last']: p['last'] = m['_ts']
    if not p['name'] and s.get('name'): p['name'] = s.get('name')
...
for uid, info in human_posters.items():
    d = len(info['days'])
    if info['messages'] == 1: status['one_time_poster'] += 1
    elif d == 1: status['single_day_poster'] += 1
    elif d >= 7: status['multi_week_active'] += 1
    else: status['few_day_active'] += 1
...
def pct_msgs(top_n):
    return round(100 * sum(v['messages'] for _, v in human_poster_list[:top_n]) / total_human_msgs, 2)
...
pareto_out = {
    'ground_truth': {
        ...
        'source': os.environ.get("GROUND_TRUTH_SOURCE", "platform admin UI, manually verified"),
    },
    'observed_in_export': {
    ...
    'posters_by_type': {
        ...
        'poster_rate_of_humans': round(100 * len(human_posters) / GROUND_TRUTH_HUMANS, 2),
    },
    'bot_footprint': {
        'total_bot_messages': sum(v['messages'] for v in bot_posters.values()),
        'bot_msg_share': round(100*sum(v['messages'] for v in bot_posters.values())/sum(v['messages'] for v in posters.values()), 2),
        'bots': [{'uid_tail': uid[-16:], 'messages': v['messages'], 'name': v['name']}
                 for uid, v in sorted(bot_posters.items(), key=lambda x: -x[1]['messages'])],
    },
    'human_concentration': {
        f'top_{n}_pct_of_msgs_by_humans': pct_msgs(n) for n in [1, 5, 10, 25, 50, 100, 200]
    },
    'human_concentration_of_membership': {
        f'top_{n}_pct_of_ground_truth_humans': pct_members(n) for n in [1, 5, 10, 25, 50, 100, 200]
    },
...
(OUT_DIR / "user_pareto_v4.json").write_text(json.dumps(pareto_out, ensure_ascii=False, indent=2))
...
stickiness_out = {
    ...
    'retention_rates_over_human_membership': {
        ...
        'one_time_posters':       round(100 * status['one_time_poster'] / GROUND_TRUTH_HUMANS, 2),
        'single_day_posters':     round(100 * status['single_day_poster'] / GROUND_TRUTH_HUMANS, 2),
        'few_day_active':         round(100 * status['few_day_active'] / GROUND_TRUTH_HUMANS, 2),
        'multi_week_active':      round(100 * status['multi_week_active'] / GROUND_TRUTH_HUMANS, 2),
    },
    'retention_rates_over_human_posters': {
        'one_time_posters':       round(100 * status['one_time_poster'] / max(len(human_posters),1), 2),
        'single_day_posters':     round(100 * status['single_day_poster'] / max(len(human_posters),1), 2),
        'few_day_active':         round(100 * status['few_day_active'] / max(len(human_posters),1), 2),
        'multi_week_active':      round(100 * status['multi_week_active'] / max(len(human_posters),1), 2),
    },
}
(OUT_DIR / "user_stickiness_v4.json").write_text(json.dumps(stickiness_out, ensure_ascii=False, indent=2))

# Temporal v4
by_day_joined = defaultdict(int)
by_day_posted = defaultdict(lambda: {'messages': 0, 'active_humans': set(), 'first_posters': 0})
seen = set()
for ts, _, _, _ in join_events:
    by_day_joined[ts.date().isoformat()] += 1
for m in sorted(content_msgs, key=lambda x: x['_ts']):
    s = m.get('sender') or {}
    if s.get('sender_type') == 'app': continue
    uid = s.get('id','')
    if not uid: continue
...
cm, cp = 0, 0
for d in all_days:
    j = by_day_joined.get(d,0); p = by_day_posted[d]
    cm += j; cp += p['first_posters']
    temporal.append({
        'date': d, 'new_members_joined': j, 'new_first_time_human_posters': p['first_posters'],
        'human_messages': p['messages'], 'active_humans': len(p['active_humans']),
        'cumulative_joined': cm, 'cumulative_human_posters': cp,
    })
(OUT_DIR / "temporal_growth_v4.json").write_text(json.dumps(temporal, indent=2, ensure_ascii=False))

print("=== GROUND-TRUTH-ANCHORED ANALYSIS ===")
print(f"Live membership (Feishu UI): {GROUND_TRUTH_MEMBERS:,}  ({GROUND_TRUTH_HUMANS:,} humans + {GROUND_TRUTH_BOTS} bots)")
print(f"Observed join events:         {join_events_observed:,} ({unique_joiners_observed:,} unique names)")
print(f"Implied departures:           ~{unique_joiners_observed - GROUND_TRUTH_HUMANS:,} over 20 days")
print()
print(f"Human posters:                {len(human_posters):,}  ({round(100*len(human_posters)/GROUND_TRUTH_HUMANS,1)}% of humans)")
print(f"Silent humans (lurkers):      {silent_humans:,}  ({round(100*silent_humans/GROUND_TRUTH_HUMANS,1)}% of humans)")
print(f"Bot posters:                  {len(bot_posters)}    ({sum(v['messages'] for v in bot_posters.values())} messages = {round(100*sum(v['messages'] for v in bot_posters.values())/sum(v['messages'] for v in posters.values()),1)}% of message volume)")
print()
print("HUMAN POSTER ENGAGEMENT:")
print(f"  multi_week_active (7+ days):  {status['multi_week_active']:,}  ({round(100*status['multi_week_active']/len(human_posters),1)}% of posters, {round(100*status['multi_week_active']/GROUND_TRUTH_HUMANS,2)}% of all humans)")
print(f"  few_day_active (2-6 days):    {status['few_day_active']:,}  ({round(100*status['few_day_active']/len(human_posters),1)}% of posters)")
print(f"  single_day_poster:            {status['single_day_poster']:,}  ({round(100*status['single_day_poster']/len(human_posters),1)}% of posters)")
print(f"  one_time_poster:              {status['one_time_poster']:,}  ({round(100*status['one_time_poster']/len(human_posters),1)}% of posters)")
print()
print("MESSAGE CONCENTRATION (human posters over human msgs):")
for n in [1, 5, 10, 25, 50, 100, 200]:
    print(f"  Top {n:>3}: {pct_msgs(n):>6}% of msgs, {pct_members(n):>6}% of 3,119 humans")
print()
print("PEAKS:")
