...
print(f"[stream_c] {len(chunks_to_do)}/{len(chunks)} chunks to process (concurrency={CONCURRENCY})", flush=True)
...
import time
t0 = time.time()
with ThreadPoolExecutor(max_workers=CONCURRENCY) as ex, CACHE_FILE.open('a') as cache_fh:
    futures = {ex.submit(extract_from_chunk, i, c): i for i, c in chunks_to_do}
    for fut in as_completed(futures):
        idx, facts = fut.result()
        all_facts[idx] = facts
        cache_fh.write(json.dumps({'chunk_idx': idx, 'facts': facts}, ensure_ascii=False) + '\n')
        cache_fh.flush()
        completed += 1
        ...
        if completed % 5 == 0 or completed == len(chunks):
            print(f"[stream_c] {completed}/{len(chunks)} chunks done ({elapsed:.0f}s elapsed, ~{eta:.0f}s ETA)", flush=True)
...
(OUT_DIR / "extraction_errors.json").write_text(json.dumps(errors, indent=2, ensure_ascii=False))
...
print(f"[stream_c] DONE. {len(all_facts)-len(errors)} successful, {len(errors)} errors", flush=True)
