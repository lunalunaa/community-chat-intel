...
def main() -> None:
    ...
    # Build report
    md = [
        f"# Brand Audit — {PRODUCT_NAME} (Example Run)",
        "",
    ...
    md += [
        "",
        "## Key Observations (example commentary — replace with your own findings)",
        "",
    ...
    md += [
        "---",
        "",
        "## Stream C Structured Brand_Confusion Incidents",
        "",
    ...
    res_counts = Counter(
        i.get("resolution", "unknown") for i in facts["brand_confusion"]
    )
    ...
    for i in random.sample(
        facts["brand_confusion"], min(40, len(facts["brand_confusion"]))
    ):
    ...
    TOK_PAT = re.compile(
        r"(token|消耗|costs?|费用|tokens?消耗|几十|几百|几千万|烧钱|太贵|expensive|太费|耗费|爆表|超出|超标|burn)",
        re.IGNORECASE,
    )
    AMT_PAT = re.compile(
        r"(\d+(?:\.\d+)?)\s*(?:万|千万|亿|k|m|千|百万|万亿)?\s*(?:token|美元|元|块|刀|rmb|\$|￥|¥)",
        re.IGNORECASE,
    )
    ...
    md = [
        f"# Token Consumption / Cost Complaints — {PRODUCT_NAME}",
        "",
        f"Total hits (keyword-filtered): {len(tok_hits):,}",
        "",
        "## Sample: 40 random hits",
        "",
    ]
    random.seed(1)
    ...
    product_cost = [
        i
        for i in facts["pricing_complaints"]
        if isinstance(i.get("service"), str)
        and PRODUCT_NAME.lower() in i["service"].lower()
    ]
    md += [
        "",
        "---",
    ...
    print(
        f"[c] wrote {csv_out} ({csv_out.stat().st_size // 1024} KB, {len(facts['messaging_intent'])} rows)",
        flush=True,
    )
