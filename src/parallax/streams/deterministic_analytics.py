...
def main() -> None:
        ...
        if mm_inv:
            ...
            join_events.append(
                (
                    ts,
                    mm_qr.group(1).strip(),
                    m_qr.group(1).strip() if m_qr else None,
                    "qr",
                )
            )
    ...
    pareto_out = {
        ...
        "observed_in_export": {
            ...
            "inferred_net_departures_20d": unique_joiners_observed
            - GROUND_TRUTH_HUMANS,
        ...
        "posters_by_type": {
            ...
            "lurker_rate_of_humans": round(
                100 * silent_humans / GROUND_TRUTH_HUMANS, 2
            ),
        ...
        "bot_footprint": {
            ...
            "bots": [
                ...
                for uid, v in sorted(
                    bot_posters.items(), key=lambda x: -x[1]["messages"]
                )
            ],
        },
        "human_concentration": {
    ...
    print("PEAKS:")
    print(
        f"  Most new members:   {max(temporal, key=lambda x: x['new_members_joined'])}"
    )
