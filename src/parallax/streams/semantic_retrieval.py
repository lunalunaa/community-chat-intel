...
def main() -> None:
        ...
        messages.append(
            {
                ...
                "content": content[
                    :1500
                ],  # embed first 1500 chars to stay in model ctx
    ...
    if embs is None:
        print(
            "[stream_b] embedding... (this is the slow part, ~2-5 min on CPU)",
            flush=True,
        )
        import time
    ...
    QUERIES = QUERIES_BY_LANGUAGE.get(
        TARGET_LANGUAGE, QUERIES_BY_LANGUAGE.get("en", {})
    )
    ...
    print(
        f"[stream_b] retrieval done → {OUT_DIR / 'retrieval_results.json'}", flush=True
    )
    ...
    for query_name, data in retrieval_out.items():
        ...
        for h in data["hits"]:
            prompt_lines.append(
                f"[{h['rank']}] (score={h['score']:.2f}) {h['content']}"
            )
        prompt_lines += [
            "",
