...
Where the platform doesn't reliably emit "member left" events (Feishu/Lark is one such platform), total group membership should be verified directly against the platform's admin UI at export time and passed in via `GROUND_TRUTH_HUMANS` / `GROUND_TRUTH_BOTS` (see `stream_d_v4.py`). This figure supersedes any membership count derived from join-event system messages, since join counts alone always overstate current membership when leave events aren't logged.
...
- `analyze.py` — keyword-stats pipeline (produces report.md)
- `topics.py` — Stream A classifier
- `stream_b_embed_retrieve.py` — Stream B embedder + synthesizer
- `stream_c_fact_extract.py` — Stream C fact extractor
- `stream_c_retry.py` — tolerant-JSON retry for failed chunks
- `stream_d_v4.py` — ground-truth-anchored deterministic analysis
- `cross_stream_synthesis.py` — final narrative synthesis
- `post_analysis.py` — brand audit, token drill, CSV exports
- `build_final_report.py` — this report generator
